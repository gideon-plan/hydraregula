# Choice/Life Adoption Plan: spregula
...
1. Delete `src/spregula/lattice.nim`
2. Move `BridgeError* = object of CatchableError` to `src/spregula/adapter.nim`
...
4. In every file importing lattice:
   ...
   - Replace `Result[T, E].bad(e[])` with `bad[T]("spregula", e.msg)`
   - Replace `Result[T, E].bad(BridgeError(msg: "x"))` with `bad[T]("spregula", "x")`
